...
namespace Daemonizer
{
    public partial class ProcessService : ServiceBase, IServiceProcess
    {
        ...
        public bool InBetweenDaysInclusive(DayOfWeek CurrentDay, DayOfWeek StartDay, DayOfWeek EndDay)
        {
            if (StartDay <= EndDay)
            {
                // Test 1 range: StartDay to EndDay
                return (StartDay <= CurrentDay && CurrentDay <= EndDay);
            }
            else
            {
                // Test 2 ranges: StartDay to 6, 0 to EndDay
                return (StartDay <= CurrentDay || CurrentDay <= EndDay);
            }
        }
        ...
        private void EvaluateSchedule()
        {
            ...
            {
                // Are we within the Scheduled runtime for the pending event?
                if (InBetweenDaysInclusive(DateTime.Now.DayOfWeek, pendEvent.Event.StartDayOfTheWeek, pendEvent.Event.EndDayOfTheWeek))
                {
        ...
        void EvaluateProcesses()
        {
            ...
            foreach (RunningProcess rp in processes)
            {
                ...
                // Outside of Schedule runtime or past time on end day.
                if (!InBetweenDaysInclusive(DateTime.Now.DayOfWeek, rp.Event.StartDayOfTheWeek, rp.Event.EndDayOfTheWeek) ||
                    (DateTime.Now.DayOfWeek == rp.Event.EndDayOfTheWeek && time > endTime))
                {
                        if (!rp.Exited)
                        {
                            StopProcess(rp.Process);
